...
description: Use when performing the repository-bound SpecBound micro-spec-author
...
Use this skill only for the `micro-spec-author` task kind when repository-derived lifecycle state is one of: `approved`.
...
- Paths: `.specbound/micro-specs/req-*/ms-*-*.md`
...
1. Verify the exact approved Requirement and selected acceptance criteria.
2. Write only the bound Micro-SPEC candidate and keep deferred work explicit.
3. Resolve target-binding and selected-AC-coverage evidence.
4. Return an agent-result that requests candidate review.

## Verification

- Resolve `target-binding` as `required`; not-applicable is forbidden.
- Resolve `selected-ac-coverage` as `required`; not-applicable is forbidden.
