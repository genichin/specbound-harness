...
description: Use when performing the repository-bound SpecBound discovery-author role
...
metadata:
  ...
  specbound:
    ...
    - .specbound/discoveries/dcy-*-r*.md
...
Use this skill only for the `discovery-author` task kind when repository-derived lifecycle state is one of: `draft`.
...
- Paths: `.specbound/discoveries/dcy-*-r*.md`
...
1. Read user intent and repository context without assuming a lifecycle decision.
2. Bind the exact Discovery target and edit only its candidate path.
3. Resolve target-binding and discovery-readiness evidence.
4. Return an agent-result that requests candidate review.

## Verification

- Resolve `target-binding` as `required`; not-applicable is forbidden.
- Resolve `discovery-readiness` as `required`; not-applicable is forbidden.
